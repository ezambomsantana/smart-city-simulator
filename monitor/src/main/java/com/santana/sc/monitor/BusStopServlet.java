...
public class BusStopServlet extends HttpServlet {
	...
	private static final List<BusStop> listBusStops = new ArrayList<BusStop>();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Files.walk(Paths.get("/home/eduardo/sc-monitor/locations/stops")).forEach(filePath -> {
				...
				try {
					...
					for (int temp = 0; temp < nList.getLength(); temp++) {
						...
						if (nNode.getNodeType() == Node.ELEMENT_NODE) {
							...
							int pas = Integer.parseInt(eElement.getElementsByTagName("pas").item(0).getTextContent());
		...
		String json = new Gson().toJson(listBusStops);
