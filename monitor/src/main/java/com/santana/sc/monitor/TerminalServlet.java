...
public class TerminalServlet extends HttpServlet {
	...
	private static final List<Terminal> listTerminal = new ArrayList<Terminal>();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Files.walk(Paths.get("/home/eduardo/sc-monitor/locations/terminals")).forEach(filePath -> {
				...
				try {
					...
					for (int temp = 0; temp < nList.getLength(); temp++) {
						...
						if (nNode.getNodeType() == Node.ELEMENT_NODE) {
							...
							//String passagers = eElement.getElementsByTagName("passagers").item(0).getTextContent();
							...
							Terminal building = new Terminal();
							building.setLat(lat);
							building.setLon(lon);
							//building.set(water);
							listTerminal.add(building);
		...
		String json = new Gson().toJson(listTerminal);
