...
actSpontaneous( State ) ->
	...
	case Action < 9998 of
		...
		true -> case is_to_generate_passagers( State ) of

			true ->
				NewState = generate_passagers( State ),
	
				?wooper_return_state_only( NewState );
			false -> 
				executeOneway( State, scheduleNextSpontaneousTick )
			end;			
		false -> case is_to_generate_bus (State) of
...
-spec generate_passagers( wooper:state() ) -> oneway_return().
generate_passagers( State ) ->

	NumPassagers = 10 + class_RandomManager:get_positive_integer_gaussian_value(_Mu=30, _Sigma=5.0 ),
	
	NewState = setAttribute( State, passagers, NumPassagers ),

	Filename = text_utils:format(
				 "/home/eduardo/sc-monitor/locations/terminals/~s.xml",
	...
	file_utils:write( InitFile, "<passagers> ~w </passagers>", [ NumPassagers  ] ),
...
-spec is_to_generate_passagers( wooper:state() ) -> boolean().
is_to_generate_passagers( State ) ->
...
-spec is_to_generate_bus( wooper:state() ) -> boolean().
is_to_generate_bus( State ) ->

	CurrentTick = class_Actor:get_current_tick( State ),

	case ?getAttr(next_bus_tick) of
...
create_car( State ) ->	
	...
	CarName = io_lib:format( "~s_bus_~B", [ BuildingName , CarCount ] ),

	NewState = class_Actor:create_actor( class_Bus,
		[ _CarName=CarName,
	...
	CurrentTick = class_Actor:get_current_tick( NewNewState ),

	TickDuration = class_Actor:convert_seconds_to_non_null_ticks(
					 NextConsuption, _MaxRelativeErrorForTest=0.50, NewNewState ),

	TickState = setAttribute( NewNewState, next_bus_tick,
								 CurrentTick + TickDuration ),	
